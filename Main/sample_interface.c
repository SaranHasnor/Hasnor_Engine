...
void initSampleInterface(void)
{
	...
	*port = 5875;
	...
	Engine.UI.staticLabel(quickString("Connectez-vous a un serveur"), staticPlacement(5, 0, 250, 50), ANCHOR_TOP_LEFT);

	Engine.UI.staticLabel(quickString("Votre nom:"),  staticPlacement(5, 35, 200, 50), ANCHOR_TOP_LEFT);
	temp = Engine.UI.textField(staticPlacement(100, 30, 150, 30), FIELDTYPE_TEXT);
	Engine.UI.setTextFieldValue(temp, 0, 0, (void**)&name, true);

	Engine.UI.staticLabel(quickString("Addresse du serveur:"), staticPlacement(275, 35, 200, 50), ANCHOR_TOP_LEFT);
	...
	Engine.UI.staticButton(quickString("Connexion"), staticPlacement(900, 20, 150, 60), connect);
	
	Engine.UI.staticButton(quickString("Quitter"), relativePlacement(0.9f, 0.8f, 0.05f, 0.1f), quit);
	...
	Engine.UI.staticButton(quickString("Envoyer"), 1, Engine.UI.WIDTH - 90, 30, 80, 30, send);

	newButton(quickString("Se deconnecter"), 1, Engine.UI.WIDTH - 160, Engine.UI.HEIGHT - 30, 150, 20, leave);*/
